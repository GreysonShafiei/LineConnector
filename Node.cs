...
namespace Connect.Core
{
    public class Node : MonoBehaviour
    {
        ...
        public void SetColorForPoint(int colorIdForSpawnedNode)
        {
            ...
            point.GetComponent<SpriteRenderer>().color =
                GameplayManagement.Instance.NodeColors[colorId % GameplayManagement.Instance.NodeColors.Count];
        }
        ...
        public void SetEdge(Vector2Int offset, Node node)
        {
            ...
            {
                ConnectedEdges[node] = top;
                return;
            }

            if (offset == Vector2Int.down)
            {
                ConnectedEdges[node] = bottom;
                return;
            }

            if (offset == Vector2Int.right)
            {
                ConnectedEdges[node] = right;
                return;
            }

            if (offset == Vector2Int.left)
            {
                ConnectedEdges[node] = left;
                return;
            }
        }
        ...
        public void SolveHighlight()
        {
            ...
            if (checkingNodes.Count == 2)
            {
                ...
                highlight.GetComponent<SpriteRenderer>().color =
                    GameplayManagement.Instance.GetHighLightColor(colorId);
            }
            else
            {
                highlight.SetActive(false);
        ...
        public bool IsDegreeThree(List<Node> resultNodes)
        {
            ...
            for (int i = 0; i < directionCheck.Count; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    ...
                    if (GameplayManagement.Instance._nodeGrid.TryGetValue(checkingPos, out Node result))
                    {
